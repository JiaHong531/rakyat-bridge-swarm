...
RakyatBridge is an autonomous, multi-agent system designed to bridge the **"Information Gap"** in Malaysian public services. It accepts unstructured local dialects (Penang Hokkien, Manglish, SMS slang), translates them using a dual-layer NLP pipeline, securely retrieves complex government policies via local MCP servers using **BM25 probabilistic RAG**, and recursively simplifies the results to a 5th-grade reading level — then translates the answer *back* into the citizen's original dialect.
